...
class Tableau(Case):
    ...
    def obtenir_voisins(self, rangee_x, colonne_y):
        ...
        #Générer la liste des coordonnées valides des cases voisine. Le tuple voisinage est là pour vous aider

        # TODO: vérifier si cette fonction fonctionne bien 
        if rangee_x <= self.dimension_rangee and rangee_x >= 1 and colonne_y <= self.dimension_rangee and colonne_y >= 1:

            while i < 8:

                tuple_obtenu = voisinage[i]
                
                x_obtenu = tuple_obtenu[0]
                y_obtenu = tuple_obtenu[1]

                x_obtenu += rangee_x
                y_obtenu += colonne_y

                if x_obtenu <= self.dimension_rangee and x_obtenu >= 1 and y_obtenu <= self.dimension_rangee and y_obtenu >= 1:

                    liste_coordonnees_cases_voisines.append((x_obtenu, y_obtenu))
    ...
    def initialiser_tableau(self):
        ...
        i, j, liste_voisins = 0, 0, []

        while i < self.nombre_mines:

            tuple_obtenu = random.choice(list(self.dictionnaire_cases.keys()))
            

            if self.dictionnaire_cases[tuple_obtenu].est_minee ==  False:

                self.dictionnaire_cases[tuple_obtenu].est_minee = True

                i += 1

                liste_voisins = self.obtenir_voisins(tuple_obtenu[0], tuple_obtenu[1])

                while j < len(liste_voisins):

                    self.dictionnaire_cases[liste_voisins[j]].ajouter_une_mine_voisine()
    ...
    def valider_coordonnees_a_devoiler(self, rangee_x, colonne_y):
        ...
        """  
        # TODO: vérifier si la fonction fonctionne comme il le faut

        if rangee_x <= self.dimension_rangee and rangee_x >= 1 and colonne_y <= self.dimension_colonne and colonne_y >= 1:

            if self.est_devoilee == True:
    ...
    def contient_mine(self, rangee_x, colonne_y):
        ...
        if self.dictionnaire_cases[(rangee_x, colonne_y)].est_minee == True:
...
def test_devoiler_case():
    ...
    tableau_test = Tableau()
    dimension_x, dimension_y = tableau_test.dimension_rangee, tableau_test.dimension_colonne
    case_1 = tableau_test.obtenir_case(dimension_x-1, dimension_y)
    case_2 = tableau_test.obtenir_case(dimension_x, dimension_y)
    case_3 = tableau_test.obtenir_case(dimension_x+1, dimension_y)

    tableau_test.devoiler_case(dimension_x-1, dimension_y)
    tableau_test.devoiler_case(dimension_x+1, dimension_y+1)

    assert case_1.est_devoilee == True
    assert not case_2.est_devoilee == True
    assert not case_3.est_devoilee == True
...
def test_case_contient_mine():
    ...
    case_1 = tableau_test.obtenir_case(dimension_x-1, dimension_y)
    case_2 = tableau_test.obtenir_case(dimension_x, dimension_y)
    case_3 = tableau_test.obtenir_case(dimension_x, dimension_y+1)

    case_1.est_minee = True
    case_2.est_minee = False

    assert tableau_test.contient_mine(dimension_x-1, dimension_y)
    assert not tableau_test.contient_mine(dimension_x, dimension_y)
    assert not tableau_test.contient_mine(dimension_x, dimension_y+1)
